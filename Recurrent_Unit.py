...
class GRU:
    def __init__(self, Mi, Mo, activation, state=None):
        self.Mi = Mi
        self.Mo = Mo
        self.f  = activation
    ...
    def __getstate__(self):
        return tuple([p.get_value() for p in self.params])

    def __setstate__(self, state=None):
        if state:
            Wxr, Whr, br, Wxz, Whz, bz, Wxh, Whh, bh, h0 = state
        else:
            Wxr, Whr, br, Wxz, Whz, bz, Wxh, Whh, bh, h0 = init_weight(self.Mi, self.Mo), init_weight(self.Mo, self.Mo), np.zeros(self.Mo), init_weight(self.Mi, self.Mo), init_weight(self.Mo, self.Mo), np.zeros(self.Mo), init_weight(self.Mi, self.Mo), init_weight(self.Mo, self.Mo), np.zeros(self.Mo), np.zeros(self.Mo)
