...
class RNN_Model:
    ...
    def fit(self, X, Y, epochs=5, mu=0.9, reg=0., batch_sz=64, lr=0.002):
...
class CharPredictNNModel:
    ...
    def sample_formation(self, text):
        text = re.sub(r'[^\!\?`\-,\.\:;"\'<>\{\}\[\]\+_\(\)\&\%\$\@\^#\*0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ]', ' ', text)
        text = re.sub(r'[ \t\n]{2,}', ' ', text)
        t_size = len(text)
        X , Y = [], []
        for i in range(0, t_size - self.seq_length - 1):
            X.append([self.map_vect[j] for j in text[i: i + self.seq_length]])
            Y.append([self.map_vect[j] for j in text[i + 1: i + self.seq_length + 1]])
        return X, Y
    ...
    def load(self, filename):
        self.seq_length, self.hidden_lay_sz, state = pickle.load(open(filename, 'rb'))
        self.compile()
